...
const DonateForm = ({ disclaimerOne, disclaimerTwo }) => {
  ...
  const isValid = () => {
    if (
      ...
      amount > 0 &&
      nameInputRef.current?.value.length > 1 &&
    ...
    if (
      ...
      amount > 0 &&
      businessEmailInputRef.current?.value.length > 1 &&
      businessEmailInputRef.current.value.length > 1 &&
      abnInputRef.current.value.length > 1
    ) {
      return true;
    }
    return false;
      ...
      <div className="mb-6">
        <label className="block text-gray-500">
          ...
          <span className="text-sm">
            Keep Me Updated With What&rsquo;s Going On At Rebound
          </span>
        </label>
      </div>
      <div className="mb-12">
