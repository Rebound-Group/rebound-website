...
const HomeExpandableGridItem = ({ data }) => {
  ...
  const borderColor = () => {
    return colorMap[data.image_border_color] || "none";
  };
  return (
    <div
    ...
    >
      ...
      <div
        className="hover: cursor-pointer flex justify-end m-2 p-2 mb-0 pb-0"
        style={{ borderTop: "1px solid #939393" }}
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <p
          className="mr-4 mb-0 font-semibold uppercase text-sm"
          style={{ color: "#939393" }}
        >
        ...
        <Icon
          icon={
            isExpanded ? "dashicons:arrow-up-alt2" : "dashicons:arrow-down-alt2"
          }
          className="border rounded-full border-melon text-melon"
        />
      </div>
    </div>
  );
